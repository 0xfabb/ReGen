...
const Dashboard = () => {
  ...
  const recentActivities = [
    { id: 1, type: "Recycled", item: "Plastic bottles", amount: "2.5 kg", date: "Today", points: 15 },
    { id: 2, type: "Composted", item: "Food waste", amount: "1.8 kg", date: "Yesterday", points: 12 },
    { id: 3, type: "Recycled", item: "Paper", amount: "3.0 kg", date: "2 days ago", points: 18 },
    { id: 4, type: "Reduced", item: "Plastic bags", amount: "0.5 kg", date: "3 days ago", points: 10 }
  ];
      ...
      <main className="flex-1 p-4 lg:p-6">
        ...
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-6">
          <div>
            ...
            <p className="mt-1 text-gray-600">Welcome back, {userData.name}</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
            <div className="flex items-center justify-between">
              ...
              <span className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full">{userData.rank}</span>
            </div>
            <div className="mt-2 text-2xl font-bold text-gray-800">{userData.points} pts</div>
            <div className="mt-4 flex items-center text-sm">
              <div className="flex-1">
                <div className="h-2 w-full bg-gray-200 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-gradient-to-r from-green-500 to-teal-500 rounded-full" 
                    style={{ width: `${(userData.points / (userData.points + userData.pointsToNextRank)) * 100}%` }}
                  ></div>
                </div>
              </div>
              <span className="ml-2 text-gray-500">{userData.pointsToNextRank} pts to {userData.nextRank}</span>
            </div>
          </div>
          
          <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
            <div className="text-gray-500 text-sm">Total Recycled</div>
            <div className="mt-2 text-2xl font-bold text-gray-800">{userData.wasteStats.recycled} kg</div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
            ...
            <div className="mt-2 text-2xl font-bold text-gray-800">{userData.wasteStats.composted} kg</div>
            <div className="mt-4 flex items-center text-sm text-green-600">
              <TrendingUp className="w-4 h-4 mr-1" />
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
            ...
            <div className="mt-2 text-2xl font-bold text-gray-800">{userData.wasteStats.reduced} kg</div>
            <div className="mt-4 flex items-center text-sm text-green-600">
              <TrendingUp className="w-4 h-4 mr-1" />
            ...
            <div className="divide-y">
              {recentActivities.map(activity => (
                <div key={activity.id} className="px-6 py-4 flex items-center">
                  <div className={`
                    p-3 rounded-lg mr-4
                    ${activity.type === 'Recycled' ? 'bg-blue-100 text-blue-700' : 
                      activity.type === 'Composted' ? 'bg-green-100 text-green-700' : 
                      'bg-yellow-100 text-yellow-700'}
                  `}>
                    {activity.type === 'Recycled' ? <Recycle className="w-5 h-5" /> : 
                     activity.type === 'Composted' ? <Leaf className="w-5 h-5" /> : 
                     <Trash2 className="w-5 h-5" />}
                  </div>
                  <div className="flex-1">
                    <h3 className="font-medium">{activity.type} {activity.item}</h3>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100">
                <h2 className="font-semibold text-lg text-gray-800">Upcoming Pickups</h2>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100">
                <h2 className="font-semibold text-lg text-gray-800">Eco Tips</h2>
              </div>
